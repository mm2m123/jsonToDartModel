...
$(function () {
  ...
  (function init() {
    ...
    try {
      ...
      showInfo('Load JSONEditor failed, please try reload');
    }
    ...
    function generate() {
              ...
              if (typeof element === 'boolean') {
                ...
                if (typeof element === 'string') {
                  ...
                  if (Number.isInteger(element)) {
                    toType = shouldEnhanceFaultTolerance ? `int.tryParse(json[${jsonKey}]?.toString() ?? '')` : `json[${jsonKey}]?.toInt() ?? 0`;
                    type = 'int';
                  } else {
                    toType = shouldEnhanceFaultTolerance ? `double.tryParse(json[${jsonKey}]?.toString() ?? '')` : `json[${jsonKey}]?.toDouble() ?? 0`;
                    type = 'double';
                  }
                }
              }
      ...
      let dartCode = `///\n/// Code generated by jsonToDartModel--🅴\n///\n${objToDart(jsonObj, rootClass, "")}`;
